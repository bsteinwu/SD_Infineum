...
for i in range(slots+1):                     #create shelf colors dictionary
    slot_name = 'Slot_'+str(i)+'_Color'
    shelf.append(slot_name)
    Shelf_Colors = dict.fromkeys(shelf, empty_color)

book_name = 'test1.xlsx'
...
for i in range(2,47):
    if ws.cell(row = i, column = 2).value is None:
        continue
    Shelf_Colors['Slot_' + str(i-1) + '_Color'] = full_color           # set color for filled slots
        ...
        if n == 0:
            ...
            for i in range(2,47):
                if current_sample == 'Slot_' + str(i-1):
                    if Shelf_Colors['Slot_' + str(i-1) + '_Color'] == empty_color:
